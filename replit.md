...
- **Signal extractor confidence/marginal_evidence (§6.5)**: Both EN and ES prompts now request `confidence` (`"high"|"medium"|"low"`, English literals only — even in ES) and `marginal_evidence` per signal alongside `quality` and `extracted_text`. `parseSignalResult` keeps both fields optional and warns once per call when any of the 5 signals omits `confidence`. `SignalScore` type adds optional `confidence?: "high"|"medium"|"low"`. The reasoning-signals endpoint already forwards both when present.
- **Decision dimension tooltips (B3)**: `client/src/components/DecisionDimensionsEditor.tsx` no longer holds a duplicate `DIMENSION_TOOLTIP_KEYS` constant — all five dimension descriptions resolve via `t('decisionDimensions.tooltip*')` from `client/src/lib/translations.ts`. Added per-option `HoverCard` tooltips on both primary and secondary `SelectContent` (HelpCircle icon inside each `SelectItem`, with `stopPropagation` on click/pointerdown so the icon doesn't auto-select the option through Radix Select). Secondary dimension also got a label-level tooltip, conditionally rendered when a secondary is selected.
